...
class EncoderLayer(nn.Module):
    def __init__(self, n_features=100, head=5, feed_forward_dim = 64, hid_linear_part_dim = 64, n_predictions = 10):
    ...
    def forward(self,X):
        ...
        if self.layer_norm1 is None:
            self.layer_norm1 = nn.LayerNorm(X.shape)
        if self.layer_norm2 is None:
            self.layer_norm2 = nn.LayerNorm(X.shape)

        
        # initialisation des paramètres d'attention pour la head 0
        ...
        for i in range(1,self.head):
            ...
            multi_head_Attention = torch.concat((multi_head_Attention, Attention_i),dim=1)
...
class Encoder(nn.Module):
    ...
    def forward(self, X):
        ...
        y = torch.FloatTensor(self.n_countries, self.n_times, self.n_features)
        ...
        for country in range(self.n_countries):
            x = self.encod1.forward(X[country]) # x.shape = [n_times, n_features]
            x = self.encod2(x)
            x = self.encod3(x)
...
class GTA_Model(nn.Module):
    def __init__(self, in_features=100, hid_features=60, out_features=100):
        super(GTA_Model, self).__init__()
        self.gcn = GCNMultiLayer(in_features, hid_features, out_features)
        self.encoder = Encoder(n_predictions=9)
